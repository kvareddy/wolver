...
WolEvalFactoryImpl::evalDiff (WolValueSptr val1, WolValueSptr val2) {
	DEBUG3_MSG << "VALUE1 " << val1->getStringRep()
	           << " VALUE2 " << val2->getStringRep()
	           << " DIFF";
	WolValueSptr compliment = evalCompliment(val2);
	WolValueSptr retValue = nullptr;
	if (compliment) {
		retValue = evalIntersection(val1, compliment);
	}
...
WolEvalFactoryImpl::evalConcatBInt(WolValueImplSptr op_val, WolValueImplSptr operand_val,
	...
	DEBUG3_MSG << "OP_VAL " << op_val->getStringRep()
			   << " OPERAND_VAL" << operand_val->getStringRep()
	           << "CONCATB IMPLYLEFT "  << implyLeft;

	assert(op_prec > operand_prec);
	WolValueImplSptr retValue = nullptr;
	if (implyLeft) {
		WolValueImplSptr splitRightValue =  evalSpliceInt(op_val, operand_prec -1, 0);
		if (evalIntersectionInt(splitRightValue, operand_val) == nullptr) {
			DEBUG3_MSG << "RESULT = EMPTY";
			return nullptr;
		}
		else {
			retValue = evalSpliceInt(op_val, op_prec -  1 , operand_prec);
			DEBUG3_MSG << "RESULT = " << retValue->getStringRep();
			return retValue;
		}
	}
		...
		if (evalIntersectionInt(splitLeftValue, operand_val) == nullptr){
			DEBUG3_MSG << "RESULT = EMPTY";
			return nullptr;
		}
		else {
			retValue = evalSpliceInt(op_val, operand_prec - 1, 0);
			DEBUG3_MSG << "RESULT = " << retValue->getStringRep();
			return retValue;
		}
	}
...
WolEvalFactoryImpl::evalConstIntersectionConst (WolValueImplSptr val1,
	...
	DEBUG3_MSG << "CONST " << val1->getStringRep()
			   ...
			   << " INTERSECTION";
	if (const_val1->_value == const_val2->_value) {
		DEBUG3_MSG << "RESULT = " << val1->getStringRep();
		return val1;
	}
	DEBUG3_MSG << "RESULT = EMPTY";
...
WolEvalFactoryImpl::evalConstIntersectionUnion (WolValueImplSptr val1,
	...
	DEBUG3_MSG << "CONST " << val1->getStringRep()
			   ...
			   << " INTERSECTION";
	if (union_val->containsValue(const_val->getLowValue())) {
		DEBUG3_MSG << "RESULT = " << val1->getStringRep();
		return val1;
	}
	DEBUG3_MSG << "RESULT = EMPTY";
...
WolEvalFactoryImpl::evalConstIntersectionConcat (WolValueImplSptr val1,
	...
	DEBUG3_MSG << "CONST " << val1->getStringRep()
			   ...
			   << " INTERSECTION";
	if(concat_val->containsValue(const_val->getLowValue())) {
		DEBUG3_MSG << "RESULT = " << val1->getStringRep();
		return val1;
	}
	DEBUG3_MSG << "RESULT = EMPTY";
...
WolEvalFactoryImpl::evalRangeIntersectionRange (WolValueImplSptr val1,
	...
	DEBUG3_MSG << "RANGE " << val1->getStringRep()
			   << " RANGE " << val2->getStringRep()
			   << " INTERSECTION";
	if (range_val1->isFull()) {
		DEBUG3_MSG << "RESULT = " << range_val2->getStringRep();
		return range_val2;
	}
	if (range_val2->isFull()) {
		DEBUG3_MSG << "RESULT = " << range_val1->getStringRep();
		return range_val1;
	}
...
WolEvalFactoryImpl::evalRangeIntersectionUnion (WolValueImplSptr val1,
	...
	if (!null) {
		if (retValue->numValues() == 1) {
			DEBUG3_MSG << "RESULT = " << retValue->getValue(0)->getStringRep();
...
WolEvalFactoryImpl::evalRangeIntersectionConcat (WolValueImplSptr val1,
	...
	WolValueImplSptr retValue = evalIntersectionInt(rangeSplit, concat_val);
	if (retValue)
	    DEBUG3_MSG << "RESULT = " << retValue->getStringRep();
	else
		DEBUG3_MSG << "RESULT = EMPTY";
...
WolEvalFactoryImpl::evalConcatIntersectionConcat (WolValueImplSptr val1,
	...
	if (firstPrecision1 < firstPrecision2) {
		WolValueImplSptr split = concat2->split(firstPrecision1);
		WolValueImplSptr retValue = evalIntersectionInt(concat1, split);
		DEBUG3_MSG << "RESULT = " << retValue->getStringRep();
		return retValue;
	}
	else if (firstPrecision1 > firstPrecision2) {
		WolValueImplSptr split = concat1->split(firstPrecision2);
		WolValueImplSptr retValue = evalIntersectionInt(split, concat2);
		DEBUG3_MSG << "RESULT = " << retValue->getStringRep();
		return retValue;
...
WolEvalFactoryImpl::evalBoolSplice (WolValueImplSptr val, int upper,
	...
	DEBUG3_MSG << "BOOL " << val->getStringRep()
	           << "SPLICE U:L =" << upper <<":" << lower;
	assert(upper == lower);
	DEBUG3_MSG << "RESULT = " << val->getStringRep();
	return val;
}
...
WolEvalFactoryImpl::evalConstSplice (WolValueImplSptr val, int upper,
	...
	DEBUG3_MSG << "CONST " << val->getStringRep()
		       << "SPLICE U:L =" << upper <<":" << lower;
...
WolEvalFactoryImpl::evalRangeSplice (WolValueImplSptr val, int upper,
	...
	DEBUG3_MSG << "RANGE " << val->getStringRep()
		       << "SPLICE U:L =" << upper <<":" << lower;
	...
	if (lower > 0)
		tempSplit = range_val->split(precision - lower);

	WolValueImplSptr finalSplit = tempSplit;
	if (upper < precision - 1)
		finalSplit = tempSplit->split(precision - upper -1);
...
WolEvalFactoryImpl::evalUnionSplice (WolValueImplSptr val, int upper,
	...
	DEBUG3_MSG << "UNION " << val->getStringRep()
		       << "SPLICE U:L =" << upper <<":" << lower;
...
WolEvalFactoryImpl::evalConcatSplice (WolValueImplSptr val, int upper,
	...
	DEBUG3_MSG << "CONCAT " << val->getStringRep()
		       << "SPLICE U:L =" << upper <<":" << lower;

	if (lower == 0 && upper == precision - 1) {
		DEBUG3_MSG << "RESULT = " << concat_val->getStringRep();
		return concat_val;
	}

	WolValueImplSptr tempSplit = concat_val;
	if (lower > 0)
		tempSplit = concat_val->split(precision - lower);
...
WolEvalFactoryImpl::evalConstCompliment (WolValueImplSptr val) {
	...
	DEBUG3_MSG << "CONSTANT = " << val->getStringRep()
			   << " COMPLIMENT";
...
WolEvalFactoryImpl::evalUnionCompliment (WolValueImplSptr val) {
	...
	for (auto i : union_val->_values) {
		WolValueImplSptr compliment = evalComplimentInt(i);
		retValue = evalIntersectionInt(retValue, compliment);
	}
...
WolEvalFactoryImpl::evalConcatCompliment (WolValueImplSptr val) {
	...
	for (int i = 0; i < size ; i++) {
		...
		WolValueImplSptr compliment = evalComplimentInt(concat_val->getValue(i));
		if (!compliment) continue;
		temp->addValue(compliment);

		WolRangeValueImplSptr temp_full_range = makeRangeImpl(total_prec - temp_prec);
		temp->addValue(temp_full_range);
		retValue->addValue(temp);
	}

	if (retValue->numValues() == 0) {
		DEBUG3_MSG << "RESULT = EMPTY";
		return nullptr;
	}
	else if (retValue->numValues() == 1) {
		DEBUG3_MSG << "RESULT = " << retValue->getValue(0)->getStringRep();
		return retValue->getValue(0);
	}
