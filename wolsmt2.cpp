...
void
WolSmt2Parser::wol_init_char_classes_smt () {
...
void
WolSmt2Parser::wol_insert_smt2 (std::string str, WolSMT2Tag type) {
...
void
WolSmt2Parser::wol_insert_symbol_smt2 (WolSMT2Node *node) {

	_symbolTable[node->getName ()] = node;
}

void
WolSmt2Parser::wol_insert_keywords_smt2 () {
	...
	wol_insert_smt2 (":diagnostic-output-channel",
			WOL_DIAG_OUTPUT_CHANNEL_TAG_SMT2);
	...
	wol_insert_smt2 (":regular-output-channel",
			WOL_REGULAR_OUTPUT_CHANNEL_TAG_SMT2);
...
void
WolSmt2Parser::wol_insert_reserved_words_smt2 () {
...
void
WolSmt2Parser::wol_insert_commands_smt2 () {
...
void
WolSmt2Parser::wol_insert_core_symbols_smt2 () {
...
void
WolSmt2Parser::wol_insert_bitvec_symbols_smt2 () {
...
void
WolSmt2Parser::wol_insert_logics_smt2 () {
...
unsigned char
WolSmt2Parser::wol_cc_smt2 (int ch) {
	if (ch < 0 || ch >= 256)
		return 0;
	return _cc[ch];
}
...
void
WolSmt2Parser::wol_savech_smt2 (char ch) {
	_saved = 1;
	_savedch = ch;
...
static
int
wol_isspace_smt2 (int ch) {
	return ch == ' ' || ch == '\t' || ch == '\r' || ch == '\n';
}

int
WolSmt2Parser::wol_read_token_aux_smt2 () {
	...
	if (ch == '#') {
		...
		for (;;) {
			...
			if (ch == '\\') {
				if ((ch = wol_nextch_smt2 ()) != '"' && ch != '\\') {
					cout << "unexpected " << ch << " after backslash '\\\\' in string"
							<< endl;
					return WOL_INVALID_TAG_SMT2;
				}
			}
		...
		for (;;) {
			...
			if (ch == '|') {
				_token.push_back ('|');
				_token.push_back (0);
				std::unordered_map<std::string, WolSMT2Node*>::iterator it =
						_symbolTable.find (_token);
				WolSMT2Node *node;
		...
		while ((wol_cc_smt2 (ch = wol_nextch_smt2 ())
				& WOL_KEYWORD_CHAR_CLASS_SMT2)) {
			assert(ch != EOF);
			_token.push_back (ch);
		}
		wol_savech_smt2 (ch);
		...
		for (;;) {
			ch = wol_nextch_smt2 ();
			if (!(wol_cc_smt2 (ch) & WOL_SYMBOL_CHAR_CLASS_SMT2))
				break;
			_token.push_back (ch);
		}
		wol_savech_smt2 (ch);
		if (_token.compare ("_") == 0)
			return WOL_UNDERSCORE_TAG_SMT2;

		std::unordered_map<std::string, WolSMT2Node*>::const_iterator it =
				_symbolTable.find (_token);
		WolSMT2Node *node;
...
int
WolSmt2Parser::wol_read_symbol (const char * str) {

	int type = wol_read_token_smt2 ();
	if (type == WOL_INVALID_TAG_SMT2)
		return 0;
	if (type == EOF) {
...
int
WolSmt2Parser::wol_read_rpar_smt2 (const char * msg) {
	int type = wol_read_token_smt2 ();
	if (type == EOF) {
...
int
WolSmt2Parser::wol_read_lpar_smt2 (const char * msg) {
	int type = wol_read_token_smt2 ();
	if (type == EOF) {
		cout << "expected '(' " << msg << " at end-of-file" << endl;
		return 0;
...
int
WolSmt2Parser::wol_parse_bitvec_sort_smt2 (int skiplu, int &width) {

	int type;

	if (!skiplu) {
		if (!wol_read_lpar_smt2 (""))
			return 0;
...
int
WolSmt2Parser::wol_declare_fun_smt2 () {

	int width = 0;
	if (!wol_read_symbol ("after 'declare-fun'"))
		return 0;

	WolSMT2Node *varNode = _lastNode;

	if (!wol_read_lpar_smt2 (" after function name"))
		return 0;
	if (!wol_read_rpar_smt2 (
	...
	if (type == WOL_BOOL_TAG_SMT2) {
		width = 1;
		varNode->setWolNode (
				WolExpFactory::getInstance ().makeVarExpr (width,
						varNode->getName ()));
	}
	else {
		if (type != WOL_LPAR_TAG_SMT2) {
		...
		if (type == WOL_UNDERSCORE_TAG_SMT2) {
			if (!wol_parse_bitvec_sort_smt2 (1, width))
				return 0;
			varNode->setWolNode (
					WolExpFactory::getInstance ().makeVarExpr (width,
							varNode->getName ()));
...
int
WolSmt2Parser::wol_parse_bitvec_term_smt2 (int type, WolNodeSptr& exp) {

	WolNodeSptr
	(WolExpFactory::*unary_func) (WolNodeSptr);
	WolNodeSptr
	(WolExpFactory::*binary_func) (WolNodeSptr, WolNodeSptr);
...
int
WolSmt2Parser::wol_parse_logic_term_smt2 (int type, WolNodeSptr& exp) {
	...
	if (type == WOL_BOOL_TAG_SMT2) {
		...
		if (exp1->getWidth () != 1) {
			cout << "unexpected bit-vector of width" << exp->getWidth ()
	    				<< "as argument to 'not'" << endl;
			return result;
		}
		exp = WolExpFactory::getInstance ().makeNotExpr (exp1);
		if (!wol_read_rpar_smt2 ("after single argument"))
			return result;
		result = 1;
	}
		...
		if (exp1->getWidth () != 1) {
			cout << "first argument of 'ite' is bit-vector of bit-width"
					<< exp1->getWidth () << endl;
		}
		else if (exp2->getWidth () != exp3->getWidth ()) {
			cout << "second argument of 'ite' is bit-vector of width"
					<< exp2->getWidth () << "third argument is bit" "vector of width"
					<< exp3->getWidth () << endl;
		}
		else {
		...
		if (!wol_read_rpar_smt2 ("after ITE arguments"))
			return result;
	}
	else {
		...
		if (nargs == 1) {
			cout << " minimum two arguments are expected for operator" << op
					<< endl;
			return result;
		}
		...
		for (WolNodeSptr arg : args) {
			if (arg->getWidth () != width) {
				cout << "arguments are of different widths for the operator " << op
						<< endl;
				del_exp = 1;
			}
		}
		...
		if (type == WOL_DISTINCT_TAG_SMT2) {
			...
			exp = WolExpFactory::getInstance ().makeTrueExpr ();
			for (int i = 0; i < (nargs - 1); i++) {
				temp = WolExpFactory::getInstance ().makeEqExpr (args[i],
						args[i + 1]);
				old = exp;
				exp = WolExpFactory::getInstance ().makeAndExpr (old, temp);
			}
			...
			if (width != 1) {
				cout << "arguments should be of width 1 for the operator " << op
						<< endl;
				result = 0;
			}
			else {
				...
				if (type == WOL_IMPLIES_TAG_SMT2) {
					for (int i = nargs - 1; i >= 0; i--) {
						if (exp) {
							old = exp;
							exp = WolExpFactory::getInstance ().makeImpliesExpr (args[i],
									old);
						}
						else {
							exp = args[i];
...
int
WolSmt2Parser::wol_parse_underscore_term_smt2 (WolNodeSptr& exp) {
...
int
WolSmt2Parser::wol_parse_let_binding_smt2 (WolNodeSptr& exp) {
	...
	if (result) {
		if (!wol_parse_term_smt2 (exp))
			result = 0;
	}

	if (result) {
		if (!wol_read_rpar_smt2 ("after let binding term"))
			result = 0;
	}
...
static std::string
convert_dec_to_bin (std::string s) {
...
int
WolSmt2Parser::wol_parse_bv_const_smt2 (WolNodeSptr& exp) {
	...
	if (!((type == WOL_SYMBOL_TAG_SMT2) && (_token[0] == 'b')
			&& (_token[1] == 'v') && (_token.size () > 2))) {
...
int
WolSmt2Parser::wol_parse_term_smt2 (WolNodeSptr& exp) {
	...
	if (type & WOL_CONSTANT_TAG_CLASS_SMT2) {
		...
		if (type == WOL_BINARY_CONSTANT_TAG_SMT2) {
			exp = WolExpFactory::getInstance ().makeConstExpr (_token.substr (2),
					2);
			return 1;
		}
...
WolSmt2Parser::wol_skip_sexprs () {
	...
	while (open > 0) {
		...
		if (type == WOL_INVALID_TAG_SMT2)
			...
			open++;
		else if (type == WOL_RPAR_TAG_SMT2)
			open--;
	}
	return 1;
...
int
WolSmt2Parser::wol_set_info_smt2 () {
	...
	if (type == WOL_STATUS_TAG_SMT2) {
		type = wol_read_token_smt2 ();
		if (type == EOF || type == WOL_INVALID_TAG_SMT2) {
			cout << "unexpected end-of-file or invalid token after ':status'"
					<< endl;
			return 0;
		}
		...
		if (!_token.compare ("sat") && !_token.compare ("unsat")
				&& !_token.compare ("unknown")) {
			cout << "invalid value " << _token << "after ':status'" << endl;
			return 0;
		}
...
int
WolSmt2Parser::wol_read_token_smt2 () {
...
int
WolSmt2Parser::wol_read_command_smt2 () {
	...
	case WOL_SET_LOGIC_TAG_SMT2:
		...
		type = wol_read_token_smt2 ();
		if (type == 0 || type == WOL_INVALID_TAG_SMT2)
			return 0;
	...
	case WOL_ASSERT_TAG_SMT2:
		...
		if (exp->getWidth () != 1) {
			cout << "assert argument is a bit-vector of length "
					<< exp->getWidth () << endl;
			return _error;
		}
		if (!wol_read_rpar_smt2 (" after asserted expression"))
			return _error;
		commands.asserts++;
		break;
	...
	case WOL_EXIT_TAG_SMT2:
		...
		if (type == WOL_INVALID_TAG_SMT2)
			return _error;
		if (type != EOF) {
			cout << "expected end-of-file after 'exit' command at " << _token
					<< endl;
			return _error;
		}
		break;
